"""
Monte Carlo Backtesting & Simulation Engine — Multi-Horizon Return Forecaster
==============================================================================
Purpose
-------
This module pairs with `walk_forward_validation.py`. Where the walk-forward
engine trains and validates a *learned* model, this module answers a
different but related question: "does the data behave the way any model
(learned or not) would assume, and what does a purely statistical simulation
say the realistic range of future returns looks like?"

Two operating modes
--------------------
1. STANDALONE MODE (no trained model): give it a list of tickers. It pulls
   real price history from yfinance, calibrates several classical stochastic
   engines (GBM, Merton jump-diffusion, Heston stochastic vol, GARCH(1,1),
   historical block bootstrap, and a weighted ensemble of all of them),
   simulates thousands of forward paths, and reports a full multi-horizon
   return distribution (mean, percentiles, VaR/CVaR, probability of profit).

2. MODEL-PAIRED MODE: if a ticker -> {horizon: predicted_return} mapping is
   available (e.g. exported from the walk-forward model as a JSON file),
   the module Bayesian-blends the model's point forecast into the Monte
   Carlo distribution rather than replacing it — so you get a full
   uncertainty band around the model's point prediction instead of a
   single number.

INTERACTIVE MODE (new)
-----------------------
Running the script with no CLI arguments (or with `--interactive`) drops
you into a guided prompt:
  1. You type in tickers (comma-separated, e.g. `AAPL,MSFT,GOOG`).
  2. The script checks whether a trained model's predictions file is
     available (either a path you provide, or the default
     `./model_predictions.json` the walk-forward engine exports to).
     - If found and it contains a forecast for a ticker/horizon, that
       ticker/horizon is run in MODEL-PAIRED MODE (Bayesian blend).
     - If not found (or a ticker/horizon is missing from it), the script
       automatically falls back to PURE MONTE CARLO using real Yahoo
       Finance price history (or the synthetic generator if yfinance/
       network isn't available).
  3. Everything else (engine choice, horizons, sim count, etc.) uses
     sensible defaults, which you can optionally override when prompted.

Realism guardrails
-------------------
Random-number simulation is only useful if it reproduces the stylized
facts of real markets: volatility clustering, fat tails, weak return
autocorrelation, strong squared-return autocorrelation, and a Hurst
exponent close to real equities (~0.5-0.6, not a pure random walk's 0.5
nor a trivially-smooth path). The `RealismValidator` in this module checks
simulated paths against the historical series on all of these
axes and produces a pass/fail realism score, so a bad calibration can't
quietly produce misleadingly narrow (or absurdly wide) forecasts.

Backtesting guardrail
-----------------------
A Monte Carlo forecast is a probabilistic statement ("70% chance the
21-day return is between -4% and +9%"). The only way to check whether that
statement is trustworthy is to test it out-of-sample, repeatedly, on real
history: `MonteCarloBacktester.rolling_origin_backtest` re-calibrates the
model using ONLY data available as of many historical origin dates, then
checks whether the realized future returns actually fell inside the
predicted distribution at the advertised rate (PIT uniformity, empirical
coverage vs nominal coverage, Kupiec POF test, CRPS scoring).

Runs standalone
-----------------
If yfinance isn't installed, there's no network access, or `--offline` is
passed, the module automatically falls back to a synthetic multi-regime
price generator (jump-diffusion + GARCH-style vol clustering) so the whole
pipeline — calibration, simulation, realism checks, backtesting, plotting —
can be smoke-tested end to end without any external dependency.
"""
